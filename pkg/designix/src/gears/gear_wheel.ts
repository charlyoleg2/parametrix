...
const pDef: tParamDef = {
	...
	params: [
		...
		pNumber('rightPressureAngle', 'degree', 20, 1, 45, 0.5),
		pNumber('leftPressureAngle', 'degree', 20, 1, 45, 0.5),
	...
	paramSvg: {
		...
		rightPressureAngle: 'gearwheel_params_main.svg',
		leftPressureAngle: 'gearwheel_params_main.svg',
...
function pGeom(t: number, param: tParamVal, suffix = ''): tGeom {
	...
	try {
		...
		const [brr1, blr1, brr2, blr2] = gwHelper.baseCircles(
			...
			param.brr2,
			param.blr2,
			param.rightPressureAngle,
			param.leftPressureAngle,
			param.involSym,
			param.involROpt,
			param.involLOpt
		);
